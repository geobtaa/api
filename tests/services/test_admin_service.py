...
class TestCacheManagementService:
    ...
    async def test_clear_cache_by_type_all(self):
        ...
        with patch('app.services.admin_service.invalidate_cache_with_prefix', new_callable=AsyncMock) as mock_invalidate:
...
class TestReindexingService:
    ...
    async def test_reindex_all_resources_success(self):
        ...
        with patch('app.services.admin_service.ENDPOINT_CACHE', True), \
    ...
    async def test_reindex_all_resources_no_cache(self):
        ...
        with patch('app.services.admin_service.ENDPOINT_CACHE', False), \
             patch('app.services.admin_service.invalidate_cache_with_prefix') as mock_invalidate, \
             patch('app.services.admin_service.reindex_resources') as mock_reindex:
...
class TestResourceProcessingService:
    ...
    async def test_get_resource_by_id_success(self, mock_resource_data):
        ...
        with patch('app.services.admin_service.database') as mock_db:
            ...
            mock_result.get = Mock(side_effect=lambda key, default=None: mock_resource_data.get(key, default))
    ...
    def test_determine_asset_info_array_value(self):
        ...
        service = ResourceProcessingService()
        resource = {"dc_format_s": "shapefile"}
        references = {"http://schema.org/downloadUrl": ["https://example.com/data1.zip", "https://example.com/data2.zip"]}
    ...
    async def test_start_summarization_task_success(self, mock_resource_data):
        ...
        with patch('app.services.admin_service.generate_resource_summary') as mock_task, \
             patch('app.services.admin_service.invalidate_cache_with_prefix', new_callable=AsyncMock) as mock_invalidate:
...
class TestAdminService:
    ...
    async def test_clear_cache(self):
        ...
        mock_cache_service = Mock()
        mock_cache_service.clear_cache_by_type = AsyncMock(return_value={"message": "Cache cleared"})
    ...
    async def test_summarize_resource(self):
        ...
        mock_resource_service.get_resource_by_id = AsyncMock(return_value={"id": "test", "title": "Test"})
