...
class TestLinkService:
    ...
    def test_init_with_various_resource_structures(self):
        ...
        test_cases = [
            {"id": "simple-resource"},
            {"id": "resource-with-refs", "dct_references_s": '{"http://schema.org/url": "http://example.com"}'},
            {"id": "resource-with-dict-refs", "dct_references_s": {"http://schema.org/url": "http://example.com"}},
...
class TestLinkServiceArcGISLinks:
    ...
    def test_get_arcgis_links_dynamic_map_layer(self):
        ...
        assert len(links) == 1
        assert {"label": "Open in ArcGIS Online (DynamicMapLayer)", "url": "http://example.com/arcgis/dynamic"} in links
    ...
    def test_get_arcgis_links_feature_layer(self):
        ...
        assert len(links) == 1
        assert {"label": "Open in ArcGIS Online (FeatureLayer)", "url": "http://example.com/arcgis/feature"} in links
    ...
    def test_get_arcgis_links_image_map_layer(self):
        ...
        assert len(links) == 1
        assert {"label": "Open in ArcGIS Online (ImageMapLayer)", "url": "http://example.com/arcgis/image"} in links
    ...
    def test_get_arcgis_links_tiled_map_layer(self):
        ...
        assert len(links) == 1
        assert {"label": "Open in ArcGIS Online (TiledMapLayer)", "url": "http://example.com/arcgis/tiled"} in links
...
class TestLinkServiceStaticMethod:
    ...
    async def test_get_resource_links_with_real_database(self):
        ...
        try:
            ...
            assert "connection" in str(e).lower() or "database" in str(e).lower() or "nodename" in str(e).lower()
...
class TestLinkServiceEdgeCases:
    ...
    def test_malformed_json_references(self):
        ...
        resource_dict = {
            "dct_references_s": '{"invalid": json, "missing": quotes}'
        }
    ...
    def test_nested_json_structures(self):
        ...
        resource_dict = {
            "dct_references_s": json.dumps(nested_refs)
        }
