...
from app.api.v1.endpoint_modules.admin import router, clear_cache, reindex, summarize_resource, identify_geo_entities
...
class TestAdminModuleStructure:
    ...
    def test_module_imports(self):
        ...
        from app.api.v1.endpoint_modules.admin import (
            router, clear_cache, reindex, summarize_resource, identify_geo_entities,
            logger
        )
...
class TestReindexEndpoint:
    ...
    async def test_reindex_function_direct(self, mock_get_service):
        ...
        mock_service = AsyncMock()
        mock_service.reindex_resources.return_value = {"status": "success", "message": "Reindexing completed"}
        mock_get_service.return_value = mock_service
...
class TestSummarizeResourceEndpoint:
    ...
    async def test_summarize_resource_function_direct(self, mock_get_service):
        ...
        mock_service = AsyncMock()
        mock_service.summarize_resource.return_value = {"status": "success", "message": "Summary generation started", "task_id": "test-123"}
        mock_get_service.return_value = mock_service
    ...
    async def test_summarize_resource_with_callback(self, mock_get_service):
        ...
        mock_get_service.return_value = mock_service
        result = await summarize_resource("rid", BackgroundTasks(), callback="cb", service=mock_service)
        from starlette.responses import JSONResponse
        assert hasattr(result, "body") or isinstance(result, JSONResponse)
...
class TestIdentifyGeoEntitiesEndpoint:
    ...
    async def test_identify_geo_entities_function_direct(self, mock_get_service):
        ...
        mock_service = AsyncMock()
        mock_service.identify_geo_entities.return_value = {"status": "success", "message": "Geographic entity identification started", "task_id": "test-456"}
        mock_get_service.return_value = mock_service
    ...
    async def test_identify_geo_entities_with_callback(self, mock_get_service):
        ...
        mock_get_service.return_value = mock_service
        result = await identify_geo_entities("rid", BackgroundTasks(), callback="cb", service=mock_service)
        from starlette.responses import JSONResponse
        assert hasattr(result, "body") or isinstance(result, JSONResponse)
