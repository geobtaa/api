...
K6_CACHE_BUST_SEARCH ?= 0
K6_ENDPOINT_BREAKDOWN ?= 0
...
K6_FRONTEND_P95_THRESHOLD_MS ?= 2500
K6_FRONTEND_P99_THRESHOLD_MS ?= 5000
K6_API_TARGET_VUS ?= 8
K6_API_RAMP_UP ?= 30s
K6_API_HOLD ?= 2m
...
k6-run: ## Internal helper for k6 targets (set K6_ENTRY=...)
	...
	@if command -v k6 >/dev/null 2>&1; then \
		...
		K6_QUERY="$(K6_QUERY)" \
		K6_QUERY_POOL="$(K6_QUERY_POOL)" \
		K6_SUGGEST_QUERY="$(K6_SUGGEST_QUERY)" \
		K6_RESOURCE_ID="$(K6_RESOURCE_ID)" \
		K6_API_KEY="$(K6_API_KEY)" \
		K6_SEARCH_PER_PAGE="$(K6_SEARCH_PER_PAGE)" \
		K6_CACHE_BUST_SEARCH="$(K6_CACHE_BUST_SEARCH)" \
		K6_CACHE_BUST_SEED="$(K6_CACHE_BUST_SEED)" \
		K6_ENDPOINT_BREAKDOWN="$(K6_ENDPOINT_BREAKDOWN)" \
		K6_SEARCH_DIAGNOSTICS="$(K6_SEARCH_DIAGNOSTICS)" \
		...
		K6_FRONTEND_P95_THRESHOLD_MS="$(K6_FRONTEND_P95_THRESHOLD_MS)" \
		K6_FRONTEND_P99_THRESHOLD_MS="$(K6_FRONTEND_P99_THRESHOLD_MS)" \
		...
		"$(DOCKER_BIN)" run --rm -i \
			...
			-e K6_CACHE_BUST_SEARCH="$(K6_CACHE_BUST_SEARCH)" \
			-e K6_CACHE_BUST_SEED="$(K6_CACHE_BUST_SEED)" \
			-e K6_ENDPOINT_BREAKDOWN="$(K6_ENDPOINT_BREAKDOWN)" \
			-e K6_SEARCH_DIAGNOSTICS="$(K6_SEARCH_DIAGNOSTICS)" \
			...
			-e K6_FRONTEND_P95_THRESHOLD_MS="$(K6_FRONTEND_P95_THRESHOLD_MS)" \
			-e K6_FRONTEND_P99_THRESHOLD_MS="$(K6_FRONTEND_P99_THRESHOLD_MS)" \
...
kamal-prime-thumbnail-cache: ## Prime thumbnail cache on Kamal
	...
	@kamal app exec -d $(KAMAL_DEST) -i --roles $(KAMAL_APP_ROLE) "bash -lc 'cd /app/backend && $(KAMAL_PYTHON) db/migrations/create_generated_visual_assets_table.py'"
	@ARGS=""; \
	if [ -n "$(PRIME_LIMIT)" ]; then ARGS="$$ARGS --limit $(PRIME_LIMIT)"; fi; \
	if [ -n "$(PRIME_BATCH_SIZE)" ]; then ARGS="$$ARGS --batch-size $(PRIME_BATCH_SIZE)"; fi; \
	if [ -n "$(PRIME_THUMBNAIL_CONCURRENCY)" ]; then ARGS="$$ARGS --concurrency $(PRIME_THUMBNAIL_CONCURRENCY)"; fi; \
...
# Prime static-map cache on remote Kamal app container.
# Usage examples:
...
kamal-prime-static-map-cache: ## Prime static-map cache on Kamal
	@echo "Priming static-map cache on Kamal (dest: $(KAMAL_DEST), role: $(KAMAL_APP_ROLE))..."
	@if [ -z "$$KAMAL_SSH_USER" ] || [ -z "$$KAMAL_HOST" ]; then \
		echo "ERROR: KAMAL_SSH_USER and KAMAL_HOST environment variables must be set."; \
		echo "$(KAMAL_DEST_HELP)"; \
		exit 1; \
...
kamal-prime-visual-caches: ## Prime thumbnail + static-map caches on Kamal
	@$(MAKE) --no-print-directory kamal-prime-thumbnail-cache \
		KAMAL_DEST="$(KAMAL_DEST)" \
		KAMAL_APP_ROLE="$(KAMAL_APP_ROLE)" \
		PRIME_LIMIT="$(PRIME_LIMIT)" \
		PRIME_BATCH_SIZE="$(PRIME_BATCH_SIZE)" \
		PRIME_THUMBNAIL_CONCURRENCY="$(PRIME_THUMBNAIL_CONCURRENCY)" \
		PRIME_FORCE="$(PRIME_FORCE)" \
		PRIME_RETRY_FAILURES="$(PRIME_RETRY_FAILURES)" \
		PRIME_RETRY_PLACEHELD="$(PRIME_RETRY_PLACEHELD)" \
		RESOURCE_IDS="$(RESOURCE_IDS)"
	@$(MAKE) --no-print-directory kamal-prime-static-map-cache \
		KAMAL_DEST="$(KAMAL_DEST)" \
		KAMAL_APP_ROLE="$(KAMAL_APP_ROLE)" \
		PRIME_LIMIT="$(PRIME_LIMIT)" \
		PRIME_BATCH_SIZE="$(PRIME_BATCH_SIZE)" \
