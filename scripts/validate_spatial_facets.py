        ...
        try:
            ...
            for resource in resources:
                try:
                    ...
                    if db_region:
                        try:
                            db_region = json.loads(db_region) if isinstance(db_region, str) else db_region
                        except (json.JSONDecodeError, TypeError):
                            db_region = None
                    
                    if db_county:
                        try:
                            db_county = json.loads(db_county) if isinstance(db_county, str) else db_county
                        except (json.JSONDecodeError, TypeError):
                            db_county = None
            ...
            if validation_results['total_checked'] > 0:
                match_rate = (validation_results['matches'] / validation_results['total_checked']) * 100
            ...
            if validation_results['mismatches'] > 0:
                ...
                if verbose:
                    ...
                    for detail in validation_results['details']:
                        logger.info(f"  {detail['resource_id']}: Country={detail['country_match']}, Region={detail['region_match']}, County={detail['county_match']}")
            else:
