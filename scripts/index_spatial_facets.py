...
async def index_all_resources(dry_run: bool = False, batch_size: int = 100):
    ...
    if "error" not in initial_stats:
        logger.info(f"Initial state: {initial_stats['indexed_resources']:,}/{initial_stats['total_resources_with_bbox']:,} resources indexed")
    ...
    if not dry_run:
        ...
        if "error" not in final_stats:
            logger.info(f"Final state: {final_stats['indexed_resources']:,}/{final_stats['total_resources_with_bbox']:,} resources indexed")
...
Examples:
  ...
  python scripts/index_spatial_facets.py --reindex-resource "p16022coll230:1750"
    ...
    parser.add_argument(
        "--reindex-resource",
        type=str,
        help="Reindex a specific resource by ID"
    )
