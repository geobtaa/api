...
Examples:
  ...
  python scripts/reindex_elasticsearch_with_spatial_facets.py --batch-size 500
    ...
    parser.add_argument(
        "--force", action="store_true", help="Force reindexing even if spatial facets are not ready"
    )
    ...
    try:
        ...
        if not args.force and not is_ready:
            logger.warning(
                "⚠️ Proceeding with reindexing despite spatial facets not being fully ready"
            )
