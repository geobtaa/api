...
- `make prime-resource-cache`: build the shared JSON:API resource representation cache used by both `/api/v1/resources/{id}` and `/api/v1/search`. Use `PRIME_LIMIT`, `PRIME_BATCH_SIZE`, `PRIME_RESOURCE_CONCURRENCY`, `PRIME_FORCE=1`, or `RESOURCE_IDS="..."` to scope/tune a run. On Kamal, use `make kamal-prime-resource-cache KAMAL_DEST=dev1`.
...
- `make kamal-prime-resource-cache`: build the shared resource representation cache inside a Kamal app container. This is useful after deploy/cache clears before collecting search/detail performance HARs.
