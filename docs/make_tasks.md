...
- `make resource-cache-init`: ensure the durable `generated_resource_representations` table exists.
...
- `make kamal-prime-static-map-cache`: build only the static-map/resource-class-icon visual cache on Kamal. Supports `PRIME_LIMIT`, `PRIME_BATCH_SIZE`, `PRIME_STATIC_MAP_CONCURRENCY`, `PRIME_FORCE=1`, and `RESOURCE_IDS="..."`.
