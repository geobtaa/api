...
async def debug_county_query():
    ...
    try:
        async with async_session() as session:
            ...
            overlapping_states = ["Minnesota", "Wisconsin", "Iowa", "Illinois", "Michigan", "Missouri", "Nebraska", "South Dakota", "Indiana", "North Dakota", "Kansas"]
            ...
            overlap_areas AS (
                SELECT wof.name,
                       ST_Area(ST_Intersection(ST_GeomFromGeoJSON(geojson.body::jsonb->>'geometry'), bbox.geom)::geography) AS overlap_area
                FROM gazetteer_wof_spr wof
                JOIN gazetteer_wof_geojson geojson ON wof.wok_id = geojson.wok_id
                CROSS JOIN bbox
                WHERE wof.placetype = 'county'
            ...
            SELECT o.name, o.overlap_area, ba.total_area, o.overlap_area / ba.total_area as overlap_ratio
            ...
            result = await session.execute(text(query), {"xmin": xmin, "ymin": ymin, "xmax": xmax, "ymax": ymax})
            rows = result.fetchall()
            print(f"County overlap areas (first 20):")
            for row in rows:
            ...
            for threshold in thresholds:
                ...
                count = result.fetchone()
                print(f"Counties meeting {threshold*100:.1f}% threshold: {count[0] if count else 0}")
