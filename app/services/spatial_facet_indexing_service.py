...
class SpatialFacetIndexingService:
        ...
        try:
            async with self.async_session() as session:
                ...
                while offset < total_count:
                    ...
                    result = await session.execute(batch_query, {"limit": self.batch_size, "offset": offset})
                    batch_resources = result.fetchall()
                    
                    if not batch_resources:
                        break
                    
                    logger.info(f"Processing batch {offset // self.batch_size + 1}: resources {offset + 1}-{offset + len(batch_resources)} of {total_count}")
                    ...
                    logger.info(f"Batch completed in {batch_time:.2f}s - Success: {batch_stats['successful']}, Failed: {batch_stats['failed']}, Skipped: {batch_stats['skipped']}")
    ...
    async def _process_batch(self, batch_resources: List[Tuple], session: AsyncSession, dry_run: bool = False) -> Dict[str, Any]:
        """
        ...
        for resource_id, dcat_bbox in batch_resources:
            try:
                ...
                existing_query = text("SELECT resource_id FROM resource_spatial_facets WHERE resource_id = :resource_id")
                existing_result = await session.execute(existing_query, {"resource_id": resource_id})
                ...
                insert_data = {
                    ...
                    "geo_country": json.dumps(spatial_facets.get("geo.country")) if spatial_facets.get("geo.country") else None,
                    "geo_region": json.dumps(spatial_facets.get("geo.region", [])) if spatial_facets.get("geo.region") else None,
                    "geo_county": json.dumps(spatial_facets.get("geo.county", [])) if spatial_facets.get("geo.county") else None
                }
        ...
        try:
            async with self.async_session() as session:
                ...
                return {
                    ...
                    "indexing_progress": (indexed_resources / total_resources * 100) if total_resources > 0 else 0
                }
