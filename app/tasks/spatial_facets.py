...
def index_spatial_facets_batch(self, resource_ids: List[str], batch_id: str = None) -> Dict[str, Any]:
    """
    ...
    """
    logger.info(f"Starting spatial facet indexing batch {batch_id} with {len(resource_ids)} resources")
...
async def _index_batch_async(resource_ids: List[str], batch_id: str = None) -> Dict[str, Any]:
    ...
    logger.info(f"Completed spatial facet indexing batch {batch_id}: {stats['successful']} successful, {stats['failed']} failed, {stats['skipped']} skipped")
    return stats
...
def index_all_spatial_facets(self, batch_size: int = 100, max_workers: int = 4) -> Dict[str, Any]:
    ...
    """
    logger.info(f"Starting spatial facet indexing for all resources (batch_size={batch_size}, max_workers={max_workers})")
...
async def _reindex_resource_async(resource_id: str) -> Dict[str, Any]:
    ...
    try:
        async with async_session() as session:
            ...
            if not resource:
                return {"status": "error", "resource_id": resource_id, "error": "Resource not found"}
            
            if not resource.dcat_bbox:
                return {"status": "error", "resource_id": resource_id, "error": "Resource has no dcat_bbox"}
