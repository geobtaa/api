...
describe('getFacetValueDisplayLabel', () => {
    ...
    it('renames georeferenced values "1"/"true"', () => {
        ...
        expect(getFacetValueDisplayLabel(item1, 'gbl_georeferenced_b')).toBe('Georeferenced');
        expect(getFacetValueDisplayLabel(itemTrue, 'gbl_georeferenced_b')).toBe('Georeferenced');

        // Also check legacy ID
        expect(getFacetValueDisplayLabel(item1, 'georeferenced_agg')).toBe('Georeferenced');
    });
    ...
    it('renames georeferenced values "0"/"false"', () => {
        ...
        expect(getFacetValueDisplayLabel(item0, 'gbl_georeferenced_b')).toBe('Not georeferenced');
        expect(getFacetValueDisplayLabel(itemFalse, 'gbl_georeferenced_b')).toBe('Not georeferenced');
    });
